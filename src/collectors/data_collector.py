...
class DataCollector:
    ...
    def get_klines_data(self, symbol: str, interval: str, start_time: int, end_time: int) -> List[List]:
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                logger.info(f"成功获取 {len(data)} 条数据: {symbol} {interval}")
                return data
    ...
    def collect_historical_data(self, symbol: str, interval: str, years_back: int = 3) -> pd.DataFrame:
        ...
        Returns:
            包含历史数据的DataFrame
        """
        logger.info(f"开始收集 {symbol} {interval} 过去 {years_back} 年的数据")
        ...
        while current_start < end_time:
            # 计算当前批次的结束时间
            current_end = min(current_start + 1000 * self._get_interval_ms(interval), end_time)
